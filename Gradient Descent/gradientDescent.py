...
plt.scatter(X_filtered_normalized, y_filtered, color='blue', label='Actual')
   plt.plot(X_filtered_normalized, y_pred, color='red', label='Predicted')
plt.xlabel('Normalized SepalLengthCm')
plt.ylabel('PetalLengthCm')
plt.title('Actual vs Predicted values (Polynomial Degree {})'.format(degree))
plt.legend()
plt.show()

# SAMPLE INPUTS:
# Select a range for input variable SepalLengthCm (e.g., 4.3 to 7.9)
# Enter the minimum value for SepalLengthCm: 4.5
# Enter the maximum value for SepalLengthCm: 7.0
